...
type ExtendedMaterialConfig = {
  ...
  uniforms?: Record<
    string,
    THREE.IUniform<unknown> | THREE.ColorRepresentation | number
  >;
...
function extendPhysicalMaterial(cfg: ExtendedMaterialConfig) {
  ...
  const defines =
    (physical as THREE.ShaderLibShader & {
  ...
  Object.entries(cfg.uniforms ?? {}).forEach(([key, uniform]) => {
    if (uniform && typeof uniform === "object" && "value" in uniform) {
      uniforms[key] = uniform as THREE.IUniform<unknown>;
      return;
    }

    uniforms[key] = { value: uniform };
  });
...
const MergedPlanes = forwardRef<THREE.Mesh, PlaneNoiseProps>(
  ({ material, width, count, height }, ref) => {
    ...
    useFrame((_, delta) => {
      ...
      const timeUniform = shaderMaterial.uniforms.time;

      if (timeUniform) {
        timeUniform.value = Number(timeUniform.value) + 0.1 * delta;
      }
    });
...
function DirLight({ position, color }: DirLightProps) {
  ...
  useEffect(() => {
    ...
    const camera = light.shadow.camera as THREE.OrthographicCamera | undefined;
    if (!camera) {
      return;
    }
